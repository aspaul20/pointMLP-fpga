...
def parse_args():
    ...
    parser.add_argument('--num_classes', default=40, type=int, choices=[15, 40], help='training on ModelNet10/40')
...
def main():
    ...
    test_loader = DataLoader(ScanObjectNN(partition='test', num_points=args.num_points), num_workers=0,
    ...
    criterion = cal_loss
    net = net.to(device)
    checkpoint_path = args.checkpoint#os.path.join(args.checkpoint, 'best_checkpoint.pth')
    checkpoint = torch.load(checkpoint_path, map_location=torch.device('cpu'))
    new_checkpoint = checkpoint 
    new_checkpoint['net'] = {}
    for key, val in checkpoint['net'].items():
        new_checkpoint['net']['module.'+key] = val 
